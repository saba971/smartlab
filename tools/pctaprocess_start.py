...
class com_pcta(object) :
    ...
    def open_pcta(self):
        ...
        try : 
            ...
            run_pcta = "sudo "+ pcta_path + "/" + self.exec_cmd
            print run_pcta 
...
if __name__ == '__main__':
    ...
    parser.add_option("--command", dest="command",default='', help="pcta.exe command")
    parser.add_option("--prompt", dest="prompt",default='%', help="pcta.exe command")

    (options, args) = parser.parse_args(sys.argv[1:])
    ###Create a PCTA Session
    try:
        PCTA_SESSION = com_pcta(options.linux_ip, options.port, options.linuxuser, options.linuxpass, options.directory, options.command, options.prompt,'available')
    except Exception as inst:
        keyword_name = "create pcta session failed"
        raise AssertionError("%s:%s-> fail to create PCTASESSION, exception: %s" \
    ...
    try:
        ...
        keyword_name = "start pcta process failed"
        raise AssertionError("%s:%s-> fail to start PCTA process, exception: %s" \
        % (__name__,keyword_name, str(inst)))
    ...
    try:
        ...
        keyword_name = "kill pcta process failed"
        raise AssertionError("%s:%s-> fail to close pcta session, exception: %s" \
        % (__name__,keyword_name, str(inst)))      
    else:
        logging.debug("disconnect pcta connection success " )       
